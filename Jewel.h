...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
		...
		void DisplayBoard (CeJeweledPP board)  // debug-only function.  Eventually I'll have graphics.
		{
				for (int row=0;row<numberOfRows;row++) {
					printf("row %d: ",row+1); // I seem to remember I was having a terrible time getting cin and cout to work.
					for (int col=0;col<numberOfCols;col++) {
